#!/usr/bin/env bash
# shensh-e2e-zig.sh — end-to-end tests for the shensh-in-Zig shell (M5).
#
# IDENTICAL to shensh-e2e.sh except BIN points at the Zig shensh executable
# (built via the named `shensh` step; run from the .zig-cache/o/<hash>/shensh
# artifact — install-to-prefix always fails PermissionDenied in rattan).
# The harness itself is binary-agnostic: it pipes shell command LINES through
# the REAL REPL (stdin pipe) and greps the combined output for expected
# patterns.  Command lines are exercised exactly like real usage; the '('
# escape lines run through the bundled flat-Shen reader/compiler.
#
# Must run from the repo root: the shell boots shell/*.shen relative to cwd
# (the script cds itself).  Requires the Zig exe + globals.csexp.
#
# $0 / argv[0]-sensitive expectations are normalized to match the Zig exe's
# invocation path (a .zig-cache/o/<hash>/shensh relative path, NOT ./shensh).
#
# Prompt pollution: the prompt ("<cwd>> ") and heredoc continuation prompts
# ("> ") interleave with command output on the same line, so expectations are
# grep patterns, not exact-output matches.
set -u
cd "$(dirname "$0")/.."

# BIN: the Zig shensh exe.  Default = ReleaseSafe artifact (host-side stable).
# Override with SHENSH_ZIG_BIN=<path> to point at Debug/ReleaseFast etc.
BIN="${SHENSH_ZIG_BIN:-zig/.zig-cache/o/60d13f0486c5bf8c5c17548aff554be9/shensh}"
BUNDLE=globals.csexp
if [ ! -x "$BIN" ]; then echo "FAIL: $BIN not built (zig build shensh)"; exit 1; fi
if [ ! -f "$BUNDLE" ]; then echo "FAIL: $BUNDLE missing (run make bundle)"; exit 1; fi

# $0 for the Zig exe = the invocation path (relative .zig-cache/o/<hash>/shensh).
BIN0="$BIN"
BIN0PAT=$(printf '%s' "$BIN0" | sed 's/[.[\*^$()+?{|]/\\&/g')
...
D="$(mktemp -d /tmp/shensh-e2e-zig.XXXXXX)"
trap 'rm -rf "$D"' EXIT
...
t  "\$0 is the invocation path"  'echo X$0X'            "X${BIN0PAT}X"
...
cp "-c default \$0 is argv0" 'echo [$0]'         ''              "\[${BIN0PAT}\]"
...
echo
echo "shensh-e2e-zig: $pass passed, $fail failed"
[ "$fail" -eq 0 ] || exit 1
exit 0
