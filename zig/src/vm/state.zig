...
pub const Vm = struct {
    ...
    /// String-stream registry (M1, streams.zig) — Vm-owned fixed array of 8
    /// {data,len,pos} slots + count, mirroring C's file-scope string_streams[]
    /// / n_string_streams (zincvm.c:361-362).  Zero-initialized; the stream
    /// prims read/write it via the stream value's (idx+1) int-in-pointer.
    ...
    pub fn loadBundle(self: *Vm, buf: [:0]const u8) i32 {
        ...
        // M1: wired to the REAL std fds 0/1/2 (the C FILE* for stdin/stdout/
        // stderr), so the stream prims and the future front-end's raw
        // read/write on fd 0/1/2 share one unbuffered path.
