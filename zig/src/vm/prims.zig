...
//! tuples/symbols/misc).  The stream I/O prims landed with M1 (streams.zig);
//! eval-kl with M2 (marshal.zig + hostcall.zig — the bundle-driven
//! compile+run chain).  The process subsystem (exec-plan/wait/kill/cd/
//! getcwd/getpid/getenv/setenv/glob) landed with M3 in execplan.zig and is
//! appended at the END of the table below.  STILL OMITTED: the dead
...
pub const prim_table = [_]PrimDef{
    ...
    // ---- stream I/O (M1, streams.zig) — prims.def order after "emptylist":
    // write-byte(2), read-byte(1), read-file-as-string(1), open(2), close(1).
    ...
    // ---- eval-kl (M2, marshal.zig + hostcall.zig) — prims.def order after
    // "error-to-string": the bundle-driven compile+run chain.
    ...
    // ---- process subsystem (M3, execplan.zig; C zincvm.c:1955-2700) ----
    .{ .name = "exec-plan", .arity = 1, .func = execplan.primExecPlan },
    .{ .name = "wait", .arity = 1, .func = execplan.primWait },
    .{ .name = "kill", .arity = 2, .func = execplan.primKill },
...
// =====================================================================
//  'e': eval-kl — the bundle-driven compile+run chain (M2)
// =====================================================================
