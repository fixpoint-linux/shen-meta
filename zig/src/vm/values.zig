...
pub fn valLambda(
    ...
    if (env_len > 0) {
        ...
        const ec_is_oldgen = g.inOldgen(@intFromPtr(env_copy));
        @memcpy(env_copy[0..len], env_root.?[0..len]);
        // M5 fix: barrier the copied env elements when env_copy lands in
        // old-gen (nursery full).  Mirrors applyBundledN / interp.zig apply —
        // an unbarriered old-gen array holding nursery pointers goes stale at
        // the next scavenge (verify_collects whole-stack rooting proof).
        if (ec_is_oldgen) {
            var j: usize = 0;
            while (j < len) : (j += 1) {
                if (gc.scan.valueReferencesNursery(g, &env_root.?[j])) {
