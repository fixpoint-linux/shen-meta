#!/usr/bin/env node
/**
 * scripts/ssg.mjs — static-site-generator build step for the shen-meta docs site.
 *
 ...
 *      VM + xterm.js assets from docs/ -> dist/.
 *
...
// (/shen/...) because the 5 pages live at different URL depths. @mfe/core and
...
const IMPORT_MAP = `{
  "imports": {
    ...
    "shen-landing": "/shen/shell/mfe/shen-page.js",
    "shen-architecture": "/shen/shell/mfe/shen-page.js",
    "shen-build": "/shen/shell/mfe/shen-page.js",
    "shen-primitives": "/shen/shell/mfe/shen-page.js",
    "shen-playground": "/shen/shell/mfe/shen-page.js",
...
<script type="module" src="/shen/shell/shell.js"></script>
</body>
</html>
`;
}
...
async function main() {
  ...
  // Copy the wasm VM assets colocated at dist/ (emscripten resolves
  // shen-wasm.wasm relative to the script's own directory).
  const wasmAssets = ['shen-wasm.js', 'shen-wasm.wasm'];
  ...
  // Copy the xterm.js terminal assets from docs/vendor/xterm/ to dist/vendor/xterm/.
  const xtermAssets = ['xterm.js', 'xterm.css'];
  for (const asset of xtermAssets) {
    const src = join(DOCS_DIR, 'vendor', 'xterm', asset);
    if (existsSync(src)) {
      mkdirSync(join(DIST, 'vendor', 'xterm'), { recursive: true });
      cpSync(src, join(DIST, 'vendor', 'xterm', asset));
      log(`  copied vendor/xterm/${asset}`);
    }
  }
