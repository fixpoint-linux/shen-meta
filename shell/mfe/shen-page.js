// shell/mfe/shen-page.js — the shen-meta docs pages (all 5 content pages)
// as ONE @mfe MFE module backed by a single Elm bundle.
//
// Five data-mfe slots (shen-landing, shen-architecture, shen-build,
// shen-primitives, shen-playground) all resolve to THIS module via the import
// map. The playground slot additionally mounts the <shen-playground> custom
// element for the interactive wasm C VM + xterm.js terminal demo.
//
// Import the custom element module to register <shen-playground> before Elm renders.
import './shen-playground.js';
//
...
// STRUCTURAL, so a shared slot name across the 5 pages would send reconcile
...
// its own `Elm` and avoiding the global collision entirely.
...
function loadElm() {
  if (!elmPromise) {
    elmPromise = (async () => {
      ...
      if (!res.ok) {
        throw new Error(`shen-page: failed to fetch ${ELM_URL} (HTTP ${res.status})`);
      }
      const code = await res.text();
      const Elm = evalBundle(code);
      if (!Elm || !Elm.Main || typeof Elm.Main.init !== 'function') {
        throw new Error('shen-page: dist/elm.js did not expose Elm.Main.init');
      }
      return Elm;
    })().catch((err) => {
