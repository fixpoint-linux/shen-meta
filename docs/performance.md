...
Status: advisor review (GLM-5.2), 2026-08-06. Grounded in the **deleted C-era
implementation** (`zincvm.c`, `gc.c`, `zincvm.h`, `zinctypes.h`) and
`shen/zinc.shen`. No code changed by this document. **Retargeting note:** the C VM
has been replaced by the Zig port (`zig/src/vm/`, `zig/src/gc/`); bare
`zincvm.c:`/`gc.c:` line numbers below refer to the retired C files and are kept as
a record. Several findings are already moot in the Zig port (e.g. symbols are
interned in `zig/src/vm/symbols.zig`; the global/prims tables are hash-backed in
`zig/src/vm/tables.zig`).
...
1. **`global_get` is a linear `strcmp` scan over ~800–1200 entries**
   (`zincvm.c:375`). Every `OP_GLOBAL` and most `OP_PRIM` lookups (via the
...
2. **`vm_exec_env` allocates a 3 MB `frame_stack` per recursive call**
   (`zincvm.c:1663`). Each `eval-kl`, `trap-error`, `call_closure1/3` recursion
   grabs 65 536 × 48 B of old-gen, never freed, scanned on every full collect.
3. **`exec_primitive` is a 50-arm `if (strcmp(...))` chain**
   (`zincvm.c:668-1342`). Hit on every `OP_PRIM` and every
   `OP_APPLY`/`OP_APPTERM` with a `VAL_PRIM`.
4. **No symbol interning** — `val_symbol` `strdup`s every time (`zincvm.c:166`),
   forcing `strcmp` everywhere and leaking `sym.name` on GC.
5. **`OP_APPTERM` (tail call) allocates a fresh env array every iteration**
   (`zincvm.c:1877`). Tail-recursive loops — exactly what the metacircular
   interp does — pump the nursery.
6. **Switch dispatch** in the hot loop (`zincvm.c:1711`) with per-instruction
...
- **Problem.** `global_get` (`zincvm.c:375-387`) does a linear `strcmp` scan
  over `global_table_len` (~800 reduced / ~1216 full). `OP_GLOBAL`
  (`zincvm.c:1829-1834`) calls it on every non-local reference. `OP_PRIM`'s
  miss path falls back to `exec_primitive_valid` (another ~50-entry linear
  `strcmp` chain at `zincvm.c:636-656`). `global_set` is also linear
  (`zincvm.c:358-373`), hit by every `set` primitive during `shen.initialise`.
...
### A2. Intern symbols
- **Problem.** `val_symbol` (`zincvm.c:166-169`) `strdup`s every call. Same
  ...
  `zincvm.c:91-96` skips it), so when a symbol Value is collected the name
...
- **Problem.** 50+ `if (strcmp(name, "..."))` arms (`zincvm.c:668-1342`),
...
- **Problem.** `switch (in->op)` (`zincvm.c:1711`) with ~17 cases; one shared
...
- **Effort.** ~150 LOC mechanical, half a day. **Risk.** Requires labels-as-values
  (a C-era-only idea — Zig has no computed goto); keyed to the retired C `Opcode` enum
  (`zinctypes.h:65-72`). **Benefit.** 10–25%.

### A5. Amortize `instr_count`/bounds checks
- **Problem.** Loop head (`zincvm.c:1676-1699`) does `++instr_count; if (...>= INSTR_HARD_LIMIT)` and `if (pc<0 || pc>=cur_len)` every instruction.
...
- **Problem.** `STACK_INIT_CAP 64` (`zincvm.c:304`) — early doublings each
...
- **Problem.** `vm_exec_env` does
  ...
  (`zincvm.c:1663`) — 65 536 × 48 B = 3 145 728 B **per recursive call**.
...
- **Problem.** `OP_APPTERM` (`zincvm.c:1854-1900`) reuses the `CallFrame` but
...
- **Problem.** Every `cons` (`zincvm.c:174-194`) does 2 root-pushes + 2
...
- **Fix.** Skip the root-pin dance when both car/cdr are immediate types (number /
  ...
  check already exists (`zincvm.c:104`).
- **Effort.** ~50 LOC. **Risk.** Medium. **Benefit.** 20–40% cons-overhead
  reduction plausible.

### B4. `OP_CUR` env-copy overhead
- **Problem.** `val_lambda` (`zincvm.c:199-215`) always `memcpy`s env into a
...
- **C2. AOT lowering** — the VM is a stand-in for a generated
...
- **C4. NaN-boxing** — rejected: changes the entire Value model and breaks the
  documented 40 B BiBOP size-class static asserts (`zinctypes.h:99-106`).
...
- **CR2.** `deep_equal` uses a `static int depth` counter (`zincvm.c:544`) —
  ...
  a parameter.
- **CR3.** `MAX_STRING_STREAMS = 8` fixed cap (`zincvm.c:254`); the `open`
  ENOENT fallback creates a string stream (`zincvm.c:1107-1109`) and workloads
...
- **CR4.** `VAL_SYMBOL` `sym.name` leak (`sym.name` is `strdup`'d C-heap; the
  `gc_scan_value` default branch at `zincvm.c:91-96` skips it). Real for
  `gensym`/`intern` in long REPL sessions. A2 fixes it.
- **CR5.** `instr_count` resets per `vm_exec_env` (`zincvm.c:1671`) — the
...
1. **F1 — add a benchmark harness first.** There is **no timing instrumentation**
   in the VM sources (no timing instrumentation). Run
   the old `-DZINCTEST` test binary over `globals.csexp` plus a fixed self-hosting workload under
   ...
   are all exposed in the C-era `gc.h`). ~1 hour. Without this, nothing can be measured.
...
`.kl` into the metacircular interpreter running ON the VM) is dominated by
Shen-level code (`shen/zinc.shen` compiler + `shen/interp.shen` metacircular VM),
not the native VM. Advisor review confirmed the C-VM categories above are mostly
...
accumulator via `append` → ~3-4B VM instructions → the 5B-instruction hard-limit
...
Skip runtime compilation of the OS `.kl` files entirely by pre-bundling them.
*(The runtime OS-load path this proposed sidestepping — `--repl` /
`interp-load-raw` of `.kl` into the meta-interpreter — has since been REMOVED
along with the full-OS bundle.)* The full OS is type-unsafe and was never
runnable on the release VM, and its runtime loading is out of scope. Not needed — D1
