\* =============================================================================
   ...
   C VM — the runtime has only `read-file-as-string`
   (zincvm.c) — so these two closures cannot run on that native target.  They
   are shen-scheme HOST-build/bootstrap loaders only.

   This file is intentionally NOT loaded by the bundle builder
   (serialize-reduced.shen).  The
   ...
   they never appear in globals.csexp, and the host-only `read-file`
   dynamic-apply they contain need not be lowered.  (This
